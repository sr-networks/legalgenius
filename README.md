...
- Use environment files for secrets and do not commit real keys:
  - Backend: create `.env` locally from `.env.example` and `source` it when running
  - Frontend: create `web/.env.local` locally from `web/.env.local.example`
- `.gitignore` already excludes `.env` and `.env.*` so real keys are not committed. Example files are tracked for convenience:
  - `.env.example` (root): placeholders for `CLERK_ISSUER`, `CLERK_JWKS_URL`, `DATABASE_URL`, LLM provider keys, etc.
  - `web/.env.local.example`: placeholder for `VITE_CLERK_PUBLISHABLE_KEY`
- Never paste real secrets into example files. Use placeholders like `your-nebius-api-key`.
- If a real key was ever committed or shared, rotate it immediately in the provider dashboard (Nebius/OpenRouter/etc.).
- Production: prefer a secrets manager (e.g., mounted env vars, Vault, AWS SM) over `.env` files. Ensure `API_ALLOW_ORIGINS` contains only your trusted frontends.
